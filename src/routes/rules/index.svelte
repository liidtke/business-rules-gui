<script lang="ts">
	...
	function deselectArea(){selectedArea.set(null)}
	...
	$: filteredRules = !ruleSearchField ? $storeRules : searchRules(ruleSearchField);
	...
	function searchRules(search) {
		...
		return $storeRules.filter(
			(x) => x.code.toLowerCase().includes(predicate) || x.title.toLowerCase().includes(predicate)
		);
	}
...
<div class="columns is-variable is-6">
	<div class="column">
		<div class="brules-menu box">
			...
			<div class="area">
				<a class="lbl" on:click={deselectArea}>
					Todas
				</a>
			</div>
	...
	<div class="column is-two-thirds">
		...
		{:else}
			...
			{#if selectedRule}
				<div class="box">
					<Rule bind:rule={selectedRule} bind:areas on:change={handleChangeRule} />
				</div>
			{/if}
			...
			<nav class="panel is-primary">
				...
				<div class="panel-block">
					<p class="control has-icons-left">
						<input class="input" type="text" placeholder="Buscar" bind:value={ruleSearchField} />
				...
				{#each filteredRules as rule}
					...
					<a class="panel-block" on:click={selectRule(rule)}>
